...
public class FollowService {
    ...
    public FollowResponse createPost(
            Long userId, FollowRequest followRequest, List<MultipartFile> imageFiles
    ) {
        ...
        Post post =
                Post.builder()
                        .user(user)
                        .categoryType(followRequest.categoryType())
                        .title(followRequest.title())
                        .content(followRequest.content())
                        .followType(followRequest.followType())
                        .build();
        ...
        return FollowResponse.responseWithoutContentPreview(post, imageUrls);
    }

    public FollowResponse updatePost(
            Long userId, Long postId, FollowRequest followRequest, List<MultipartFile> imageFiles
    ) {
        User user = getUser(userId);
        ...
        Post updatedPost =
                Post.builder()
                        ...
                        .title(followRequest.title()) // 새 제목 설정
                        .content(followRequest.content()) // 새 내용 설정
                        .followType(followRequest.followType())
        ...
        deleteImageFiles(followRequest.deleted());
        List<String> imageUrls = findImageUrls(updatedPost.getId());

        return FollowResponse.responseWithoutContentPreview(updatedPost, imageUrls);
    }
    ...
    public FollowResponse getPost(Long userId, Long postId) {
        getUser(userId);
        Post post =
                postRepository
                        .findById(postId)
    ...
    public PagedResponse<FollowResponse> getAllPosts(Long userId, int page, int size) {
        getUser(userId);
    ...
    public PagedResponse<FollowResponse> getPostsByFollowType(
            Long userId, FollowType followType, int page, int size
    ) {
        getUser(userId);
        Pageable pageable = PageRequest.of(page, size);
        Page<Post> postsPage =
    ...
    public PagedResponse<FollowResponse> searchPosts(
            Long userId, String keyword, int page, int size
    ) {
        getUser(userId);
        Pageable pageable = PageRequest.of(page, size);
        Page<Post> postsPage =
                postRepository.searchByKeyword(CategoryType.FOLLOW, keyword, pageable);
        return mapPostsToPagedResponse(postsPage);
    }

    private PagedResponse<FollowResponse> mapPostsToPagedResponse(Page<Post> postsPage) {
        List<FollowResponse> follows = postsPage.stream()
                        .map(FollowResponse::responseWithContentPreview)
                        .collect(Collectors.toList());

        return new PagedResponse<>(
                follows, postsPage.getTotalElements(), postsPage.getTotalPages());
    }
