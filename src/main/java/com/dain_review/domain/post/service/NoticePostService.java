...
public class NoticePostService extends AbstractPostService {
    ...
    public NoticePostService(PostRepository postRepository, UserRepository userRepository, ImageFileService imageFileService, ApplicationEventPublisher eventPublisher) {
    ...
    public PostResponse getPost(Long userId, Long postId, PostSearchRequest postSearchRequest) {
        ...
        Long prevPosId = postRepository.findPreviousPost(postId, null, CategoryType.NOTICE, null, null, postSearchRequest.keyword());
        Long nextPostId = postRepository.findNextPost(postId, null, CategoryType.NOTICE, null, null, postSearchRequest.keyword());

        // 게시물의 모든 이미지 url 리스트를 반환
        List<String> imageUrls = imageFileService.findImageUrls(post.getId(), ContentType.POST, S3PathPrefixType.S3_NOTICE_PATH);
    ...
    protected List<String> saveImages(List<MultipartFile> imageFiles, Post post) {
        S3PathPrefixType s3Path = S3PathPrefixType.S3_NOTICE_PATH;
